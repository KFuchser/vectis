...
def main():
   ...
   # 3. Strategic Filters
    ...
    with st.expander("🕵️ Data Inspector (Debug Mode)"):
        st.write("Raw Data Sample (Austin Only):")
        # Show top 20 Austin records regardless of filters
        debug_df = df[df['city'] == 'Austin'].head(20)
        st.dataframe(
            debug_df[['permit_id', 'description', 'valuation', 'velocity', 'issued_date']],
            use_container_width=True
        )
        st.caption(f"Austin Total Raw Records (Before Filters): {len(df[df['city'] == 'Austin'])}")
